...
// scripts/memory-arbor-prompt-frame.ts
import { mkdir, readFile, writeFile } from "node:fs/promises";
import { join } from "node:path";

// packages/core/src/index.ts
var DEFAULT_ROOT_NODE_ID = "root";
var DEFAULT_SLOT_CONFIGS = [
  {
    name: "memory_profile",
...
function normalizeMemoryConfig(value) {
  ...
  const slots = Array.isArray(input.slots) ? input.slots.filter((slot) => {
    if (!slot || typeof slot !== "object") return false;
    const candidate = slot;
    return typeof candidate.name === "string" && typeof candidate.purpose === "string" && typeof candidate.rootNodeId === "string" && typeof candidate.maxTokens === "number" && Number.isFinite(candidate.maxTokens) && candidate.maxTokens > 0;
  }) : defaultConfig.slots;
  return {
    slots: slots.length > 0 ? slots : defaultConfig.slots,
    injection: {
      maxMemoryTokens: typeof input.injection?.maxMemoryTokens === "number" && Number.isFinite(input.injection.maxMemoryTokens) && input.injection.maxMemoryTokens > 0 ? input.injection.maxMemoryTokens : defaultConfig.injection.maxMemoryTokens
    },
    temporaryWorkspace: {
      maxTokens: typeof input.temporaryWorkspace?.maxTokens === "number" && Number.isFinite(input.temporaryWorkspace.maxTokens) && input.temporaryWorkspace.maxTokens > 0 ? input.temporaryWorkspace.maxTokens : defaultConfig.temporaryWorkspace.maxTokens,
      pressureRatio: typeof input.temporaryWorkspace?.pressureRatio === "number" && Number.isFinite(input.temporaryWorkspace.pressureRatio) && input.temporaryWorkspace.pressureRatio > 0 && input.temporaryWorkspace.pressureRatio <= 1 ? input.temporaryWorkspace.pressureRatio : defaultConfig.temporaryWorkspace.pressureRatio
    }
  };
}
function createEmptyMemoryStore(config = defaultMemoryConfig(), timestamp = nowIso()) {
...
function normalizeMemoryStore(value, config = defaultMemoryConfig()) {
...
function readMemorySlots(store) {
...
function buildMemoryInjectionView(store, config = defaultMemoryConfig()) {
  let usedTokens = 0;
  let truncated = false;
  const slots = [];
...
function createRootNode(timestamp) {
  return {
...
function isMemoryNode(value) {
  if (!value || typeof value !== "object") return false;
  const candidate = value;
  return typeof candidate.id === "string" && typeof candidate.title === "string" && typeof candidate.summary === "string" && typeof candidate.content === "string" && Array.isArray(candidate.tags) && (candidate.status === "active" || candidate.status === "archived") && (candidate.nodeKind === "root" || candidate.nodeKind === "branch" || candidate.nodeKind === "leaf") && (typeof candidate.parentId === "string" || candidate.parentId === null) && Array.isArray(candidate.childIds) && Array.isArray(candidate.sourceRefs) && typeof candidate.tokenEstimate === "number" && Number.isFinite(candidate.tokenEstimate) && typeof candidate.version === "number" && typeof candidate.createdAt === "string" && typeof candidate.updatedAt === "string";
}
function isMemorySlot(value) {
  if (!value || typeof value !== "object") return false;
  const candidate = value;
  return typeof candidate.name === "string" && typeof candidate.purpose === "string" && typeof candidate.rootNodeId === "string" && typeof candidate.maxTokens === "number" && Number.isFinite(candidate.maxTokens) && candidate.maxTokens > 0 && Array.isArray(candidate.loadedNodeIds);
}
function ensureRootNode(store) {
  if (!store.nodes.some((node) => node.id === DEFAULT_ROOT_NODE_ID)) {
    store.nodes.unshift(createRootNode(nowIso()));
  }
}
function reconcileChildren(store) {
...
function reconcileSlots(store, config) {
...
function findActiveNode(store, nodeId) {
  return store.nodes.find((node) => node.id === nodeId && node.status === "active");
}
function loadedActiveNodes(store, slot) {
  return slot.loadedNodeIds.map((nodeId) => findActiveNode(store, nodeId)).filter((node) => node !== void 0);
}
function buildBreadcrumb(store, nodeId) {
  const breadcrumb = [];
  ...
  while (current && !seen.has(current.id)) {
    ...
    current = current.parentId === null ? void 0 : store.nodes.find((node) => node.id === current?.parentId);
  }
  return breadcrumb.reverse();
}
function buildTreePath(store, nodeId) {
...
var base = process.env.MEMORY_ARBOR_HOME || join(home, ".memory-arbor");
var storeFile = join(base, "store.json");
var configFile = join(base, "config.yaml");
var promptFrameStateFile = join(base, "prompt-frame-state.json");
...
async function readYaml(path) {
  ...
  const { parse } = await Promise.resolve().then(() => __toESM(require_dist(), 1));
...
function buildPromptFrame(delta) {
...
function memoryNodeBlock(snapshot) {
  const { node, slotNames } = snapshot;
  const tags = node.tags.length > 0 ? ` tags="${escapeXml(node.tags.join(","))}"` : "";
  return [
...
function snapshotMemoryView(view) {
  const nodes = {};
...
function buildPromptDelta(current, previous) {
...
function sameSnapshotNode(left, right) {
  return left.node.version === right.node.version && left.slotNames.join(",") === right.slotNames.join(",");
}
async function readPromptFrameState() {
  const raw = await readJson(promptFrameStateFile);
  if (typeof raw === "object" && raw !== null && "sessions" in raw && typeof raw.sessions === "object" && raw.sessions !== null) {
    return {
      version: 1,
...
function readOutputFormat(args = process.argv.slice(2)) {
  const formatArg = args.find((arg) => arg.startsWith("--format="));
  const format = formatArg?.slice("--format=".length);
  if (format === void 0 || format === "plain") return "plain";
  if (format === "claude-json") return "claude-json";
  throw new Error(`Unsupported output format: ${format}`);
}
function writeFrame(frame, format) {
  if (format === "plain") {
