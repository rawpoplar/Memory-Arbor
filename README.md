...
packages/mcp/                MCP 工具暴露层壳，后续用于工具接入。
...
在 OpenCode 完整接入中，模型之后可以通过 `memory_maintain_context` 批量创建/更新、标记、丢弃和装载记忆；也可以使用更细粒度的 `memory_*` 原子工具。
...
Claude Code 当前是 prompt hook 降级接入：通过 `UserPromptSubmit` hook 运行 `scripts/memory-arbor-prompt-frame.ts`，把 `store.json` 中已装载 slots 追加进本轮 prompt 可见上下文。
...
- skill 只描述如何消费 Memory Arbor 上下文；只有当前宿主暴露 `memory_*` 工具时才调用工具。
