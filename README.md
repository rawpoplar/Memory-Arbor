...
完整设计和后续版本计划见 [docs/design.md](docs/design.md)。
...
`.opencode/package.json` 使用 `file:` 依赖指向当前仓库内的 `packages/core`、`packages/context`、`packages/tools` 和 `plugins/opencode`。OpenCode 会在启动时为 `.opencode` 配置目录安装本地插件依赖。
...
当前假定同一个状态目录在任意时刻只由一个工具服务实例负责写入。不保证多个独立进程同时写入同一状态目录时的数据一致性。
