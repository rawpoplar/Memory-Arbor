...
- Claude Code 和 Codex 由 `memory_status` 按需加载已装载记忆。
...
它不保存状态，不实现记忆树，也不直接修改上下文。它指导模型按需调用四个高层工具，并消费 `memory_status` 返回的记忆投影和后续 prompt delta。

### Claude Code / Codex MCP 与 prompt hook

Claude Code 和 Codex 通过 MCP 接入，prompt hook 只负责后续增量。

它负责：

- 暴露 `memory_query`、`memory_apply`、`memory_status` 和 `memory_admin`。
- `memory_status` 返回当前 loaded slots 的完整 `memoryProjection`。
- `UserPromptSubmit` 在新会话只记录 loaded slots 基线，不输出完整快照；后续节点变化才输出 `<memory_arbor_prompt_delta>`。
