...
- 模型通过四个高层工具操作记忆：`memory_query`、`memory_apply`、`memory_status` 和 `memory_admin`。
- Claude Code 和 Codex 当前只通过 `UserPromptSubmit` prompt hook 追加已装载记忆。
...
`packages/tools` 保存组合记忆树与 context 的四个高层 façade 实现。
...
- 暴露 `memory_query`、`memory_apply`、`memory_status` 和 `memory_admin`。
...
它不保存状态，不实现记忆树，也不直接修改上下文。有四个高层工具时，它指导模型使用工具；没有工具时，它只指导模型消费已注入的 Memory Arbor frame。

### Claude Code / Codex prompt hook

Claude Code 和 Codex 当前是 prompt hook 降级接入。

它负责：

- 在 `UserPromptSubmit` hook 中运行 `scripts/memory-arbor-prompt-frame.ts`。
- 读取 `MEMORY_ARBOR_HOME` 或默认共享状态目录下的 `store.json` 和 `config.yaml`。
- 将已装载 slots 输出为 `<memory_arbor_prompt_frame>`，追加进本轮 prompt 可见上下文。
...
`interactionProtocolVersion` 当前为 `1`。公开工具固定为：

- `memory_query`：搜索记忆，并可按 `openIds` 展开节点。
- `memory_apply`：创建或更新节点，标记 memorized refs、丢弃 refs，并装载 slot。
- `memory_status`：读取 slots、context frame、临时工作区 pressure 和状态版本。
- `memory_admin`：归档、移动或撤销 marker 的低频修复操作。

典型记忆化流程：

1. 模型从 `<temporary_workspace>` 中读取 ref。
2. 调用 `memory_query` 避免重复，按需用 `openIds` 展开节点。
3. 调用 `memory_apply` 创建或更新节点、标记 refs、装载 slot。
4. 调用 `memory_status` 确认状态。

无价值内容清理流程：

1. 模型判断某个旧 ref 无价值。
2. 调用 `memory_apply`，把 ref 放入 `discardRefs`。
...
- 保持搜索实现位于 `memory-core`，由 `memory_query` 对外提供。
...
- `packages/tools`：四个高层 memory façade 实现。
...
node --experimental-strip-types --check "packages\tools\src\index.ts"
...
- 临时工作区超限时只提示，不裁剪。
- `memory_apply` 可以批量创建/更新节点、标记 refs、丢弃 refs 和装载 slot。
